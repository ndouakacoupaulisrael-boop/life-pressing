...
class ModifierCommandeScreen extends StatefulWidget {
  ...
  const ModifierCommandeScreen({super.key, required this.commande});

  @override
  State<ModifierCommandeScreen> createState() => _ModifierCommandeScreenState();
}

class _ModifierCommandeScreenState extends State<ModifierCommandeScreen> {
  List<Client> clients = [];
  ...
  double get totalModifie {
    return details.fold<double>(0, (total, detail) => total + detail.total);
  }
  ...
  Future<void> chargerDonnees() async {
    try {
      ...
      final listeClients = await ClientService.instance.getClients();

      final listeVetements = await DatabaseHelper.instance.getVetements();

      final listeDetails = await CommandeService.instance.getDetailsCommande(
        widget.commande.id!,
      );

      final paiements = await CommandeService.instance.getPaiementsCommande(
        widget.commande.id!,
      );
      ...
      try {
        clientCommande = listeClients.firstWhere(
          (client) => client.id == widget.commande.clientId,
        );
      } catch (_) {
        clientCommande = null;
      }
  ...
  Future<void> modifierDetail(int index) async {
    if (articlesVerrouilles) {
      return;
    }
    ...
    int nouvelleQuantite = detail.quantite;

    final couleursDisponibles = List<String>.from(couleurs);

    if (!couleursDisponibles.contains(detail.couleur)) {
      couleursDisponibles.add(detail.couleur);
    }

    String nouvelleCouleur = detail.couleur;

    final matieresDisponibles = List<String>.from(matieres);

    if (!matieresDisponibles.contains(detail.matiere)) {
      matieresDisponibles.add(detail.matiere);
    }

    String nouvelleMatiere = detail.matiere;

    final controller = TextEditingController(text: detail.quantite.toString());
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              title: Text('Modifier ${detail.vetement}'),
                    ...
                    InputDecorator(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          ...
                          isExpanded: true,
                          items: couleursDisponibles.map((couleur) {
    ...
    setState(() {
      details[index] = DetailCommande(
        id: detail.id,
        commandeId: widget.commande.id!,
        vetementId: detail.vetementId,
        vetement: detail.vetement,
  ...
  void supprimerDetail(int index) {
    if (articlesVerrouilles) {
      return;
    }
  ...
  Future<void> modifierCommande() async {
    ...
    if (dateController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('La date est obligatoire.'),
          backgroundColor: Colors.red,
        ),
      );
    ...
    if (!articlesVerrouilles && details.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
    ...
    try {
      if (articlesVerrouilles) {
        ...
        await CommandeService.instance.modifierCommande(
          commandeOriginale: widget.commande,
          clientId: client.id!,
        ...
        await CommandeService.instance.modifierCommandeComplete(
          commandeOriginale: widget.commande,
          clientId: client.id!,
          date: dateController.text,
          statut: statut,
          details: List<DetailCommande>.from(details),
        );
      }
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                          ...
                          trailing: articlesVerrouilles
                              ...
                              : Row(
                                  ...
                                  children: [
                                    IconButton(
                                      tooltip:
                                          'Modifier quantité / couleur / matière',
                                      onPressed: () => modifierDetail(index),
                                      icon: const Icon(Icons.edit),
                                    ),
                                    IconButton(
                                      tooltip: 'Retirer',
                                      onPressed: () => supprimerDetail(index),
                                      icon: const Icon(
                  ...
                  ElevatedButton.icon(
                    onPressed: enregistrement ? null : modifierCommande,
                    icon: enregistrement
                        ? const SizedBox(
                            width: 18,
                            height: 18,
