...
class DatabaseHelper {
  ...
  Future<int> insertPaiement(Paiement paiement) async {
    ...
    return db.insert('paiements', paiement.toMap());
  }
  ...
  Future<List<Paiement>> getPaiements() async {
    ...
    return List.generate(maps.length, (index) => Paiement.fromMap(maps[index]));
  }
  ...
  Future<int> deletePaiement(int id) async {
    ...
    return db.delete('paiements', where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<double> getTotalPayeCommande(int commandeId) async {
    final paiements = await getPaiementsCommande(commandeId);
  ...
  Future<int> insertDetailCommande(DetailCommande detail) async {
    ...
    return db.insert('details_commande', detail.toMap());
  }
  ...
  Future<List<DetailCommande>> getDetailsCommande(int commandeId) async {
    ...
    return List.generate(
      maps.length,
      (index) => DetailCommande.fromMap(maps[index]),
    );
  }
  ...
  Future<int> deleteDetailCommande(int id) async {
    ...
    return db.delete('details_commande', where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<double> calculerTotalCommande(int commandeId) async {
    ...
    if (resultat.isEmpty || resultat.first['total'] == null) {
      return 0;
    }
  ...
  Future<int> insertVetement(Vetement vetement) async {
    ...
    return db.insert('vetements', vetement.toMap());
  }

  Future<List<Vetement>> getVetements() async {
    final db = await database;

    final maps = await db.query('vetements');

    return List.generate(maps.length, (index) => Vetement.fromMap(maps[index]));
  }
  ...
  Future<int> deleteVetement(int id) async {
    ...
    return db.delete('vetements', where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<Database> _initDatabase() async {
    ...
    final String path = kIsWeb
        ? 'pressing.db'
        : join(await getDatabasesPath(), 'pressing.db');
  ...
  Future<void> _ajouterVetementsParDefaut(Database db) async {
    ...
    for (final nom in vetementsParDefaut) {
      ...
      if (existants.isEmpty) {
        await db.insert('vetements', {'nom': nom, 'prix': 0.0});
      }
    }
  }
 ...
 CREATE TABLE tarifs(
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 7) {
      ...
      final colonnes = await db.rawQuery('PRAGMA table_info(journal_actions)');

      final contientLue = colonnes.any((colonne) => colonne['name'] == 'lue');
    ...
    if (oldVersion < 12) {
      final colonnes = await db.rawQuery('PRAGMA table_info(tarifs)');

      final contientVetementId = colonnes.any(
        (colonne) => colonne['name'] == 'vetementId',
      );

      if (!contientVetementId) {
        await db.execute(
          'ALTER TABLE tarifs '
          'ADD COLUMN vetementId INTEGER',
        );
    ...
    if (oldVersion < 13) {
      final colonnes = await db.rawQuery('PRAGMA table_info(details_commande)');

      final contientMatiere = colonnes.any(
        (colonne) => colonne['name'] == 'matiere',
      );
  ...
  Future<List<Commande>> getCommandesParClient(int clientId) async {
    final db = await database;
    ...
    return List.generate(maps.length, (index) => Commande.fromMap(maps[index]));
  }
  ...
  Future<int> insertCommande(Commande commande) async {
    ...
    return db.insert('commandes', commande.toMap());
  }

  Future<List<Commande>> getCommandes() async {
    final db = await database;

    final maps = await db.query('commandes');
  ...
  Future<Commande?> getCommandeById(int id) async {
    ...
    final maps = await db.query('commandes', where: 'id = ?', whereArgs: [id]);
  ...
  Future<void> updateStatutCommande(int commandeId, String statut) async {
    final db = await database;
  ...
  Future<void> mettreAJourTotalCommande(int commandeId) async {
    ...
    final total = await calculerTotalCommande(commandeId);
  ...
  Future<int> deleteCommande(int id) async {
    ...
    return db.delete('commandes', where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<void> afficherPaiements() async {
    final db = await database;

    final resultat = await db.query('paiements');

    debugPrint('========== PAIEMENTS ==========');
  ...
  Future<void> afficherCommandes() async {
    ...
    final resultat = await db.query('commandes');

    debugPrint('========== COMMANDES ==========');

    for (final ligne in resultat) {
      debugPrint(ligne.toString());
    }

    debugPrint('========== FIN COMMANDES ==========');
  }

  Future<void> afficherDetailsCommande(int commandeId) async {
    final db = await database;
  ...
  Future<int> saveParametre(Parametre parametre) async {
    final db = await database;
  ...
  Future<Parametre?> getParametre() async {
    ...
    final resultat = await db.query('parametres', limit: 1);

    if (resultat.isEmpty) {
      return null;
    }

    return Parametre.fromMap(resultat.first);
  }

  Future<int> updateParametre(Parametre parametre) async {
    final db = await database;
  ...
  Future<int> insertActionJournal(ActionJournal action) async {
    final db = await database;

    return db.insert('journal_actions', action.toMap());
  }

  Future<List<ActionJournal>> getActionsJournal({int limit = 100}) async {
    final db = await database;
    ...
    return maps.map((map) => ActionJournal.fromMap(map)).toList();
  }
  ...
  Future<List<ActionJournal>> getNotificationsNonLues({int limit = 50}) async {
    final db = await database;

    final maps = await db.query(
      'journal_actions',
      where: 'autorisee = ? AND lue = ?',
      whereArgs: [0, 0],
      orderBy: 'id DESC',
      limit: limit,
    );
  ...
  Future<int> marquerActionCommeLue(int id) async {
    final db = await database;
  ...
  Future<int> marquerToutesNotificationsCommeLues() async {
    final db = await database;
  ...
  Future<int> insertCharge(Charge charge) async {
    ...
    return db.insert('charges', charge.toMap());
  }

  Future<List<Charge>> getCharges() async {
    final db = await database;

    final maps = await db.query('charges', orderBy: 'date DESC, id DESC');

    return maps.map((map) => Charge.fromMap(map)).toList();
  }
